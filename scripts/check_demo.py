...
def main() -> int:
    ...
    core_ok &= ok
    ok, _ = check("Backend", "http://localhost:8001/api/ready")
    core_ok &= ok
    ok, _ = check("Frontend", "http://localhost:3000")
    core_ok &= ok

    _, health = check("Health", "http://localhost:8001/api/health", required=False)
    if isinstance(health, dict):
        provider = health.get("llm_provider")
        llm_connected = bool(health.get("llm_connected"))
        detail = health.get("llm_detail", "")
        print(f"{'[OK]' if llm_connected else '[FAIL]'} LLM        {provider} · {detail}")
        core_ok &= llm_connected
